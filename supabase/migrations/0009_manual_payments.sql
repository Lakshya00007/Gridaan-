-- =====================================================================
-- Manual payments: COD, UPI review, and bank transfer review.
-- Historical gateway columns and enum values are intentionally retained
-- so existing orders remain readable.
-- =====================================================================

alter type public.payment_method add value if not exists 'manual_upi';
...
alter table public.orders
  ...
  add column if not exists manual_payment_sender_name text,
  add column if not exists manual_payment_note text,
  add column if not exists manual_payment_verified_at timestamptz,
  add column if not exists manual_payment_verified_by uuid references public.profiles(id) on delete set null,
  add column if not exists manual_payment_rejected_reason text;
...
-- Replace the old order RPC with a service-role-only signature that stores
-- customer-submitted manual payment details. Payment always starts pending.
...
begin
  ...
  if p_payment_method::text in ('manual_upi', 'bank_transfer')
     and nullif(btrim(p_manual_payment_reference), '') is null then
    raise exception 'Payment reference is required for manual payments';
  end if;

  if p_payment_method::text = 'bank_transfer'
     and nullif(btrim(p_manual_payment_sender_name), '') is null then
    raise exception 'Sender name is required for bank transfer';
  end if;
  ...
  values (
    ...
    'placed',
    case
      when p_payment_method::text in ('manual_upi', 'bank_transfer')
        then 'Order created; manual payment awaiting verification'
      else 'Order created'
    end
  );
...
comment on function public.create_order is
  'Service-role only. Creates COD or manual-payment orders after API validation.';

-- No active flow should be able to mark an order paid without an admin review.
drop function if exists public.mark_order_paid(uuid, text, text, text);
